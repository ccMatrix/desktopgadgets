/**
    ...
    Author: Benjamin Schirmer <codename.matrix@gmail.com>
**/

// Global instance of calendar picker
...
CalendarPicker.prototype.showAt = function(_x, _y) {
	...
	// Only if Animation lib is available
	...
	// Draw actual month grid
	this.drawMonth();

	// Only if Animation lib is available
	if (typeof(Animation) == "function") {
		var aniShow = new Animation(this.divBox);
		aniShow.setDuration(300);
		aniShow.fadeIn();
		aniShow.setElement(divbg);
		aniShow.fadeIn();
	}
	
	// Focus closeBox to handle keyboard navigation
	this.closeBox.focus();
}
...
CalendarPicker.prototype.drawMonth = function() {
	...
	for (var i=firstDay.getDate(); i<=lastDay.getDate(); i++) {
		...
		daybox.onmousemove = "__CAL_INSTANCE.switchBGColor(event.srcElement.name);";
		daybox.onmouseout = "__CAL_INSTANCE.resetBGColor(event.srcElement.name);";
...
CalendarPicker.prototype.switchBGColor = function(ele) {
	try {
		...
		this.cursorPos = parseInt(ele.replace("daybox", "").replace("yearbox", ""));
		debug.trace("Cursor changed to: "+this.cursorPos);
	}
	catch (E) {
